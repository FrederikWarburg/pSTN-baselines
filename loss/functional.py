...
def kl_div(mu, sigma, sigma_prior):
    ...
    p = dist.MultivariateNormal(loc=mu_prior, scale_tril=sigma_prior*torch.eye(len(mu_prior), device=mu.device))
    q = dist.MultivariateNormal(loc=mu, scale_tril=sigma*torch.eye(len(mu), device=mu.device))
